namespace GSA.OpenItems.Web
{
    ...
    public partial class SearchPerson : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if(hdnShow.Value=="0")
            {
                txtLastName.Attributes.Add("style", "display:none");
                txtFirstName.Attributes.Add("style", "display:none");
                btnSearch.Attributes.Add("style", "display:none");
                note.Attributes.Add("style", "display:none");
                td_fn.Attributes.Add("style", "display:none");
                td_ln.Attributes.Add("style", "display:none");
                grid_row.Attributes.Add("style", "display:none");
            }
            else
            {
                txtLastName.Attributes.Add("style", "display:");
                txtFirstName.Attributes.Add("style", "display:");
                btnSearch.Attributes.Add("style", "display:");
                note.Attributes.Add("style", "display:");
                td_fn.Attributes.Add("style", "display:");
                td_ln.Attributes.Add("style", "display:");
                grid_row.Attributes.Add("style", "display:");
            }
        ...
        void gvPersons_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                if (SearchMode == SEARCH_CONTACT)
                {
                    ...
                    e.Row.Attributes.Add("ondblclick", "javascript:on_select(" + id.ToString() + ",'" + name + "','" + phone + "');");
                }
                if (SearchMode == SEARCH_EMAIL)
                {
                    if (((DataRowView)e.Row.DataItem)[4] != DBNull.Value)                                            
                        e.Row.Attributes.Add("ondblclick", "javascript:on_email_select('" + ((DataRowView)e.Row.DataItem)[4].ToString() + "');");                    
                }
            }
        }

...
namespace GSA.OpenItems.Web
{
    ...
    public partial class Logon : PageBase
    {
        ...
        protected override void PageLoadEvent(object sender, System.EventArgs e)
        {
            ...
            var conn = Settings.Default.DefaultConn;

            var slblEnvironmentIsActive = Settings.Default.PasswordMenuIsVisible;

            var sDatabaseServer1 = Settings.Default.DatabaseServer1;
            // for db server name alias:
            var sDatabaseServer2 = Settings.Default.DatabaseServer2;

            var sDatabaseServerUAT = Settings.Default.DatabaseServerUAT;

            var sProdWebServerName = Settings.Default.ProdWebServerName;
            var sProdWebServerAlias = Settings.Default.ProdWebServerAlias;
            ...
            if (System.Web.HttpContext.Current.Session["WebServer"] != null)
            {
                ...
                txtDBServerName.Value = Session["ApplUrl"].ToString() + ", DB: " + Mid(Settings.Default.DefaultConn, 14, 53) + ", Browser: " + browser.Browser + ", Browser Type: " + browser.Type;
            }


            var sApplicationDisabled = Settings.Default.ApplicationDisabled;

            if (sApplicationDisabled)
            {
                tr_disabled.AddDisplay();
                lblDisabledMessage.Text = Settings.Default.DisabledMessage;
        ...
        void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string adPath = Settings.Default.adPath;//Path to NCR LDAP directory server
                string sDomainValue = Settings.Default.DomainValue;//ENT
                ...
                if (true == adAuth.IsAuthenticated(sDomainValue, txtUsername.Text.Trim(), txtPassword.Text.Trim(), this.Page, true, out sEmail))
                {
                    if (sEmail.Trim() != "" && sEmail != null && sEmail != "0")
                    {
                        ...
                        //TODO: Uncomment when we get working with the sproc

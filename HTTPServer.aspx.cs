namespace GSA.OpenItems.Web
{
    ...
    public partial class HTTPServer : PageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (obj_xml.DocumentElement.Name == "verify_assignment") //action code 4
                {
                    ...
                    // action code 4
                    if (Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnAssignVerification"]))
                ...
                {                    
                    ...
                    */
                    ...
                    {
                        // action code 10
                        if (Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnRerouteAssign"]))  // 10
                ...
                {                    
                    ...
                    {
                        var email_request = 0;
                        if (Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnRerouteRequest"]))
                ...
                if (obj_xml.DocumentElement.Name == "reassign_reroute") //3
                {
                    ...
                    if (intActionCode == (int)HistoryActions.haReviewerAssignment)
                    {
                        ...
                        {
                            if (Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnReviewerAssignment"]))
                        ...
                        {
                            if (Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnReviewerReassign"]))
                        ...
                        if (intActionCode == (int)HistoryActions.haRerouteRequest)
                        {
                            ...
                            else
                            {
                                ...
                                if (!User.IsInRole(((int)UserRoles.urBudgetDivisionAdmin).ToString()) &&
                                    Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnRerouteRequest"]))
                                    email_request = Emails.InsertEmailRequest(user, (int)HistoryActions.haRerouteRequest, true);
                ...
                if (obj_xml.DocumentElement.Name == "group_reroute") //action 16
                {
                    ...
                    if (Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnGroupAssign"]))
                    {
                        // action code 19
                ...
                if (obj_xml.DocumentElement.Name == "reroute") //10
                {
                    ...
                    if (intRequestID > 0)
                    {
                        ...
                        if (Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnRerouteAssign"]))
                            email_request = Emails.InsertEmailRequest(user, (int)HistoryActions.haRerouteAssignment, true);
                ...
                if (obj_xml.DocumentElement.Name == "select_doc_to_email")
                {
                    ...
                    if (selected_value != "" && selected_doc != "")
                    {
                        ...
                        if (Boolean.Parse(ConfigurationManager.AppSettings["InsertHistory_OnSelectAttachmentToCO"]))
                        {
                ...
                if (obj_xml.DocumentElement.Name == "select_doc_revision_email")
                {
                    ...
                    if (selected_value != "" && selected_doc != "")
                    {
                        ...
                        if (Boolean.Parse(ConfigurationManager.AppSettings["InsertHistory_OnSelectAttachmentToSME"]))
                        {
                            var doc = new Document(doc_id);
                            var file_name = doc.FileName;
                            var doc_type = String.Join(",", doc.DocumentTypeName);
                ...
                if (obj_xml.DocumentElement.Name == "select_doc_approved")
                {
                    ...
                    {
                        selected = (Boolean.Parse(selected_value)) ? (int)DocRevisionStatus.dsApproved : (int)DocRevisionStatus.dsNotApproved;
                        doc_id = Int32.Parse(selected_doc);
        ...
        {
            //
            if (Boolean.Parse(ConfigurationManager.AppSettings["SendEmailOnAllTypeOfAssignments"]))
            {
                ...
                var sNewReviewerEmail = "";
                    ...
                    if (intItemID != 0)
                    {
                        var ds = Item.GetDocNumByItemID(intItemID);
                        sDocNum = ds.Tables[0].Rows[0]["DocNumber"].ToString();
                    }

namespace GSA.OpenItems.Web
{
    ...
    public partial class Controls_Menu : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
                ...
                hlTabApp1.NavigateUrl = ConfigurationManager.AppSettings["OpenItemsDefaultPage"];
                hlTabApp2.NavigateUrl = ConfigurationManager.AppSettings["FundStatusDefaultPage"];
                var sAdminPageRightsForBDAdmin = ConfigurationManager.AppSettings["AdminPageRightsForBDAdmin"];
                var sAdminPageRightsForOrgAdmin = ConfigurationManager.AppSettings["AdminPageRightsForOrgAdmin"];
                var sPasswordMenuIsVisible = ConfigurationManager.AppSettings["PasswordMenuIsVisible"];
                ...
                {
                    ActivateOI_OrgAdminMenu();
                    if (sAdminPageRightsForOrgAdmin == "True")
                    {
                        Activate_AdminMenu();
                    }
                }
                ...
                if (Page.User.IsInRole(((int)UserRoles.urBudgetDivisionAdmin).ToString()))
                {
                    ...
                    if (sAdminPageRightsForBDAdmin == "True")
                    {
                        Activate_AdminMenu();
                    }
                }
                ...
                if (sPasswordMenuIsVisible.ToLower().Trim() == "true")
                {

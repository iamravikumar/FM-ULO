namespace GSA.OpenItems.Web
{
    ...
    public partial class Controls_SearchCriteria : System.Web.UI.UserControl
    {
        ...
        public void FillControls()
        {
            try
            {
                Search.Attributes.Add("onclick", "return checkOrg();");
                DataRow dr;

                var dt = FundStatus.GetBusinessLineList().Tables[0];
                ...
                ds = FundStatus.GetBAList();
                if (ds == null || ds.Tables[0].Rows.Count == 0)
                    throw new Exception("Application error. No Budget Activity found.");
                s = BudgetActivity;
                ddgBA.Table = ds.Tables[0];
                if (s == "")
                    BudgetActivity = ConfigurationManager.AppSettings["Default_BUDGET_ACTIVITY"];
                ...
                {   //init Summary Functions list:
                    lstSumFunc.Table = FundStatus.GetSummaryFunctionsList().Tables[0];
                }
                if (OCC)
                {   //init all object class codes:
                    dt = FundStatus.GetObjectClassCodeList().Tables[0];
                    dt.Columns[0].ReadOnly = true;
                    lstOCCode.Table = dt;
                }
                if (CostElem)
                {
                    dt = FundStatus.GetCostElemList().Tables[0];
                    dt.Columns[0].ReadOnly = true;
                    mgCostElem.Table = dt;
                }
            }

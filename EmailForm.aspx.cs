    ...
    public partial class EmailForm : PageBase
    {
        ...
        private void InitFields(int RecipientUserID)
        {
                ...
                switch (EmailMode)
                {
                    ...
                    case MODE_SAMPLE_CO:
                        txtTo.Text = ConfigurationManager.AppSettings["Email_COEmailAddress"];
                        txtSubject.Text = BuildCustomString(ConfigurationManager.AppSettings["Email_COSubject"]);
                        InitAttachmentsList();
                        break;
                    ...
                    case MODE_SAMPLE_SME:
                        ...
                        ddlTo.DataBind();
                        txtSubject.Text = BuildCustomString(ConfigurationManager.AppSettings["Email_SMESubject"]);
                        InitAttachmentsList();
                        break;

                    case MODE_SAMPLE_REVIEWER:
                        txtTo.Text = GetEmailAddress(RecipientUserID);
                        var file_name = Server.MapPath("~") + ConfigurationManager.AppSettings["Email_ToReviewerTextFile"];
                ...
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        if ((bool)dr[field_to_search] && docs_array.IndexOf("," + doc_in_arr) == -1)
                        {
                            ...
                            td.Attributes.Add("class", "regBldGreyText");

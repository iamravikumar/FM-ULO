namespace GSA.OpenItems.Web
{
    ...
    public class FSReviewUI
    {
        ...
        private void DrawTableContent()
        {
            ...
            foreach (DataRow dr_total in TotalsDataTable.Rows)
            {
                ...
                if (func != dr_total["SUM_FUNC"].ToString())
                {
                    ...
                    if (func != "")
                    {
                        ...
                        if (MonthlyView)
                        {
                            foreach (var m in month_arr)
                            {
                                ...
                                tr.Cells.Add(GetNewCellMoney(arr_func_totals[m_index], "tableTotal"));
                                arr_func_totals[m_index] = 0;
                            }
                        }
                        tr.Cells.Add(GetNewCellMoney(arr_func_totals[0], "tableTotal"));
                    ...
                    tr.Cells.Add(GetNewCellText(func, "tableBold", CELL_ALIGN_LEFT));
                    if (DisplayColumnObjClassCode)
                        tr.Cells.Add(GetNewCellText(dr_total["OBJ_CLASS_CD"].ToString(), "tableBold", CELL_ALIGN_RIGHT));
                    ...
                    if (MonthlyView)
                    {
                        foreach (var m in month_arr)
                        {
                            ...
                            if (BuildReportForExcel)
                                tr.Cells.Add(GetNewCellMoney(current_amount, "tableRow"));
                            else
                                tr.Cells.Add(GetNewCellMoneyLink(current_amount, m, dr_total["SUM_FUNC"].ToString(), dr_total["OBJ_CLASS_CD"].ToString()));
                        }
                    }
                ...
                else
                {
                    ...
                    tr.Cells.Add(GetNewCellText("", "tableBold", CELL_ALIGN_LEFT));
                    if (DisplayColumnObjClassCode)
                        tr.Cells.Add(GetNewCellText(dr_total["OBJ_CLASS_CD"].ToString(), "tableBold", CELL_ALIGN_RIGHT));
            ...
            if (MonthlyView)
            {
                ...
                {
                    m_index = Int32.Parse(m);
                    tr.Cells.Add(GetNewCellMoney(arr_totals[m_index], "tableFooter"));
                }
            }
            tr.Cells.Add(GetNewCellMoney(arr_totals[0], "tableFooter"));
        ...
        private void DrawTableHeader()
        {
            ...
            HtmlTableCell td;
            //SUM_FUNC:                
            tr.Cells.Add(GetNewCellText("SUM FUNC", "tableCaption", CELL_ALIGN_CENTER));
            if (DisplayColumnObjClassCode)
            {
                //OBJ CLASS CODE:      
            ...
            if (MonthlyView)
                foreach (var m in month_arr)
                {
                    ...
                    tr.Cells.Add(GetNewCellText(caption, "tableCaption", CELL_ALIGN_CENTER));
                }
            //TOTALS:                
            tr.Cells.Add(GetNewCellText("TOTAL", "tableCaption", CELL_ALIGN_CENTER));
        ...
        private HtmlTableCell GetNewCellMoneyLink(decimal Amount, string BookMonth, string Func, string OC)
        {
            ...
            td.Align = CELL_ALIGN_RIGHT;
            td.Attributes.Add("class", "tableLink");
            td.Attributes.Add("onclick", "get_link('" + String.Format(CellLinkOnClick, BookMonth, Func, OC) + "');");

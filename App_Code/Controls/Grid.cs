...
namespace Controls
{
    public class Grid : System.Web.UI.UserControl
	{
        ...
        */
		...
		{
            set { ArtDG.Attributes.Add("ondblclick", "if(__tr(event)) " + value); }
        }
        ...
        protected void Grid_Save()
        {
            ...
            for (var i = 1; i < ArtDG.Rows.Count; i++)
            {
                ArtDG.Rows[i].Attributes.Add("class", i % 2 != 0 ? ItemCSSClass : ItemCSSClass + "Alt");
                ArtDG.Rows[i].Attributes.Add("tabindex", "-" + i.ToString());
            }
            if (Sorting)
            {
